...
const displayMovements = function (acc, sort = false) {
  ...
  trans.forEach(function (x, i) {
    const type = x > 0 ? 'deposit' : 'withdrawal';
    ...
    <div class="movements__date">${displayDate}</div>
                  <div class="movements__value">$${x.toFixed(2)}</div></div>`;
...
// Displaying the  Numbers on the bottom (In,Out,Interest)
...
btnLogin.addEventListener('click', function (e) {
  e.preventDefault();
  // Determine if the login creditentials are valid
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // Create the Current Date Manual Way
    // const now = new Date();
...
const anyDeposits = transactions.some(mov => mov > 1500);
console.log * anyDeposits;

// 161
labelBalance.addEventListener('click', function () {
  const movementsIU = Array.from(
